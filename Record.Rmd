...
\begin{center}
  \includegraphics{medias/manifeste.png}
\end{center}
...
Cette étude vise à comprendre et à améliorer la fréquentation du Sèmè City Open Park à partir de l’analyse des données d’usage disponibles. Elle repose sur l’exploitation des données issues du logiciel de gestion de flux SMART SCOP, couvrant 14 809 visites réalisées par 3 540 usagers uniques sur 209 jours d’ouverture.
...
En mobilisant des analyses descriptives, comportementales et une modélisation multivariée (Random Forest expliquant près de 70 % de la variance observée), l’étude identifie des facteurs d’influence hiérarchisés, majoritairement maîtrisables par le parc. Ces résultats permettent de formuler des préconisations stratégiques concrètes, orientées vers l’activation de l’existant plutôt que la croissance externe. L’enjeu principal n’est donc pas d’attirer toujours plus d’usagers, mais de mieux accueillir, mieux orienter et mieux fidéliser ceux qui franchissent déjà les portes du Sèmè City Open Park.
...
Les renseignements issus de l’analyse convergent vers un constat clair : les leviers d’amélioration de la fréquentation sont internes, actionnables et à priori peu coûteux.

Trois axes prioritaires peuvent être activés immédiatement :

- 1. Sécuriser la première visite : La rétention initiale constitue le levier le plus critique. Structurer l’onboarding, clarifier le parcours d’accueil et instaurer un suivi systématique après la première visite permettrait de transformer une part significative des visiteurs occasionnels en usagers récurrents.

- 2. Rééquilibrer l’usage des espaces : La fréquentation actuelle repose sur un nombre limité d’espaces. Encourager la découverte et la circulation entre espaces existants permettrait d’augmenter la fréquentation globale sans accroître la pression sur les zones déjà saturées.

- 3. Exploiter les temps faibles sans dégrader l’expérience : Certains jours et créneaux horaires présentent un fort potentiel latent. Des activations légères et ciblées permettraient de lisser la fréquentation dans le temps, en optimisant l’usage de la capacité existante.
...
Cette étude repose sur 02 sources de données principales. La première, SMART SCOP, un logiciel interne au SCOP qui nous aide à retracer les flux d'entrée sortie du SCOP. C'est le régistre numérique qu'utilisent les usagers lorsqu'ils visitent le Sèmè City Open Park. De cet outil, nous avons extrait au format excel, deux bases essentielles : 
...
Le tableau ci-dessous présente les résultats de l'harmonisation des variables textuelles de la base `data_usagers`. « Valeurs » indique le nombre de valeurs uniques détectées avant et après harmonisation.
...
kable(
  ...
  format = "pipe",
  caption = "Résultats de l'harmonisation des variables textuelles") %>%
  kable_styling(
...
La seconde base, `data_usagers_comportement`, agrège ces visites au niveau usager et contient `r nrow(data_usagers_comportement)` individus uniques caractérisés par 30 variables comportementales (nombre de visites, durées moyennes, espaces fréquentés, régularité, etc).
...
# Bienvenu au Sèmè City Open Park

Sèmè City est l'un des projets phares du gouvernement béninois qui entend développer une économie du savoir dynamique et inclusive. Sèmè City se positionne comme un hub incontournable dédié à la formation, la recherche et l'innovation ainsi qu'à l'incubation et la création d'entreprises innovantes. Le Bénin souhaite ainsi former une nouvelle génération de talents, les équiper pour l'avenir et favoriser un modèle de croissance inclusive et durable fondé sur « l' innovation Made in Africa (IMA) ». Sèmè City est conçu et mis en œuvre par I' Agence de Développement de Sèmè City (ADSC), établissement public doté de la personnalité morale et de l'autonomie financière. L'ADSC développe un cadre favorable et attractif pour différent.e.s opérateur.trice.s et partenaires dans le but de construire cet écosystème d'innovation, créateur de richesses et d'emplois. L'objectif d'ici 2030 est de créer plus de 100000 emplois, dont plus d'un tiers d'auto-emplois et 40% par des femmes.


```{r}
knitr::include_graphics("medias/5Y6A4442.jpg")
```

Situé à quelques pas du campus principal Sèmè One, le SCOP (Sèmè City Open Park) est un espace d'innovation ouvert de 2 500 m2, conçu pour démocratiser l'accès à la recherche, à l'expérimentation et à la fabrication numérique. Il offre un environnement dynamique favorisant l'apprentissage, le prototypage et la collaboration, accessible dès l'âge de 8 ans.

Construit à partir de containers maritimes recyclés, le SCOP incarne une approche durable et modulable de l'innovation. Il met à disposition une large gamme d'outils technologiques avancés, incluant un parc de fabrication et de prototypage rapide (impression 3D, découpe laser, électronique/lOT), ainsi qu'un laboratoire spécialisé dans les techniques du rayonnement X.
Bien plus qu'un espace technique, le SCOP est un véritable hub de créativité et d'échange destiné à un public varié : lycéens, étudiants, chercheurs, entrepreneurs, entreprises, artisans, bricoleurs et makers issus du secteur formel et informel. Ce lieu favorise la montée en compétence et la matérialisation d'idées innovantes en nouveaux produits et solutions.


```{r}
knitr::include_graphics("medias/3d_workshop.JPG")
```

Le SCOP repose sur trois piliers fondamentaux :

- Démocratisation des nouvelles technologies : Sensibiliser et former à la Conception Assistée par Ordinateur (CAO/FAO) à travers des ateliers interactifs et des démonstrations, accessibles dès 8 ans.
- Prototypage numérique et fabrication rapide : Accompagner les porteurs de projets dans la transformation d'idées en prototypes fonctionnels grâce à un accès aux équipements de pointe.
- Communauté et collaboration : Favoriser les échanges entre innovateurs de tous horizons, en créant un écosystème de partage via des espaces de coworking, des événements et des rencontres inspirantes.

À travers ces initiatives, le SCOP s'affirme comme un catalyseur de l'innovation Made in Africa, offrant aux talents locaux les moyens de concevoir, d'expérimenter et d'inventer les solutions de demain. La fréquentation du Sèmè City Open Park n’est donc pas un problème à résoudre, mais une dynamique à lire, à accompagner et à amplifier.


# Orientation stratégique de l'analyse

Toute analyse de données implique des choix. Et choisir, c’est renoncer. Dans le cadre de cette étude, ces choix n’ont pas été dictés par la facilité, mais par une volonté claire : produire une analyse utile, actionnable et alignée avec les enjeux réels du Sèmè City Open Park.

## Durée considérée pour l’analyse

Bien que le SCOP opère des activités depuis avril 2022, l’analyse s’est concentrée sur une période de `r periode$nb_jours_total`, du `r periode$date_debut` au `r periode$date_fin`. Cela correspond à la période d'exploitation de l'outil SMART SCOP, déployé plus tôt cette année.
...
Pourquoi ne pas être remonté plus loin ? En réalité, nous dispositions en version papier de plusieurs régistres qui renseignent les flux d'usagers depuis l'ouverture, et d'archives de listes de présence aux activités clés menées sur le site depuis 2022. Une démarche d'extraction des données a été initiée au moment de mon stage, donnant naissance à une base de données d'activité, que nous avons finalement dû écarter de cette étude.

## Suppression de la base de données d’activité

Ce choix peut sembler contre-intuitif. Pourtant, il répond à une logique claire. Premièrement, à peine 38 % des usagers enrégistrés dans SMART SCOP auraient réellement participé aux activités sur la période considérée (avril à décembre 2025). Cela réflète une sous-utilisation de l'outil, pourtant dédié à la gestion du flux d'usagers. Utiliser cette base de données d'activité aurait généré une perte énorme d'informations en cas de jointures.

De plus, l’objectif de l’étude n'était pas d’évaluer la performance intrinsèque des activités proposées, mais de comprendre les dynamiques globales de fréquentation indépendamment des formats spécifiques. Intégrer trop tôt la dimension “activité” aurait risqué de brouiller la lecture, en confondant :

- l’effet du lieu ;
- l’effet de la programmation des activités ;
...
Les observations effectuées révèleront que l’enjeu principal n’est pas tant l’attraction ponctuelle que la transformation des primo-visiteurs en usagers récurrents.
...
Le moins que l'on puisse dire, c'est que le rythme de fréquentation au Sèmè City Open Park est très dynamique. Les visites quotidiennes oscillent entre 0 (les jours de fermetture comme le dimanche) et 182 visites par jour, avec une moyenne générale à environ 71 visites par jour (calculée sur les jours effectifs d'activité).
...
La fréquentation du Sèmè City Open Park n’est pas constante. Elle suit des cycles nets, observables à l’échelle mensuelle, hebdomadaire et journalière.

### Journées les plus fréquentées

Les analyses temporelles montrent que la fréquentation est majoritairement concentrée en semaine, avec 83,6 % des visites, contre 16,4 % le week-end.

```{r}
knitr::include_graphics("outputs/figures/03_frequentation_jour_semaine.png")
```

Le vendredi apparaît comme le jour le plus actif, avec 2 894 visites cumulées, tandis que le dimanche est de loin le jour le moins fréquenté, avec seulement 95 visites sur l’ensemble de la période.
...
```{r}
knitr::include_graphics("outputs/figures/05_heatmap_jour_heure.png")
```
...
Le Sèmè City Open Park compte 29 ateliers et espaces actifs, mais leur fréquentation est loin d’être homogène. Les résultats montrent une forte concentration sur très peu d'espaces :
...
La population du parc est majoritairement jeune avec un âge médian de 23 ans et une moyenne de 25 ans. La distribution révèle une forte concentration dans la tranche 20-25 ans qui représente à elle seule 50,2% des usagers.

```{r}
knitr::include_graphics("outputs/figures/09_pyramide_ages.png")
```

Cette caractéristique s'explique par la proximité d'établissements universitaires, notamment Epitech et l'Université d'Abomey-Calavi (UAC), qui constituent le principal réservoir d'usagers.
...
287 établissements différents sont recensés, témoignant d'une diversité remarquable. Cependant, le taux de renseignement de 44,2% indique qu'une part importante des usagers ne déclare pas leur affiliation, ce qui peut biaiser l'analyse des profils.
...
Le travail personnel est le motif le plus utilisé par les usagers du Sèmè City Open Park, qui bénéficient sans doute de l'expérience de base du SCOP, parfaite pour le coworking : un kit de démarrage qui leur permet d'accéder à un espace de travail libre et confortable, de l'électricité et une connexion internet.
...
De la cartographie de la fréquentation du Sèmè City Open Park, plusieurs régularités fortes émergent. Ces constats empiriques permettent de formuler trois hypothèses de recherche structurantes, qui guideront l’analyse des facteurs d’influence et la discussion des résultats.
...
Ces trois hypothèses structureront la suite de l’analyse. Elles seront confrontées aux résultats statistiques, comportementaux et prédictifs issus de la modélisation afin d’en tester la validité et d’en tirer des enseignements opérationnels.
...
importance_vars <- tibble::tibble(
  ...
  `%IncMSE` = c(35.317, 19.884, 11.688, 5.887, 5.607),
  IncNodePurity = c(47969, 20350, 15456, 2229, 13060),
  ImportanceRelative = c(35.005, 19.709, 11.584, 5.835, 5.558),
  Catégorie = c(
    "Démographique",
    "Comportemental",
    "Historique",
...
importance_vars %>%
  mutate(
    Variable = dplyr::recode(
      Variable,
      nb_nouveaux_usagers = "Nouveaux usagers",
      nb_espaces_actifs   = "Nombre d’espaces actifs",
      nb_visiteurs_ma7    = "Visiteurs (moyenne mobile 7 jours)",
      jour_semaine_num    = "Jour de la semaine",
      nb_visiteurs_sd7    = "Variabilité des visiteurs (7 jours)"
    )
  ) %>%
  arrange(desc(`%IncMSE`)) %>%
  kable(
    caption = "Variables clés expliquant la fréquentation du Sèmè City Open Park",
    digits = 2,
    booktabs = TRUE,
    align = "lrrrl",
    format = "pipe"
...
La variable la plus influente est le nombre de nouveaux usagers (`nb_nouveaux_usagers`), représentant à elle seule environ 35,32 % de l’importance totale, suivie par le nombre d’espaces actifs (`nb_espaces_actifs`, 19,88 %) et la fréquentation moyenne des sept derniers jours (`nb_visiteurs_ma7`, 11,69 %). Ces trois variables concentrent près de 66 % du pouvoir explicatif du modèle, indiquant que la fréquentation repose principalement sur des dynamiques internes cumulatives.
...
importance_cat <- tibble::tibble(
  ...
  ImportanceTotale = c(37.3, 26.5, 24.0, 13.1),
  NombreVariables = c(3, 5, 3, 5),
  Pourcentage = c(36.9, 26.3, 23.8, 13.0)
)

importance_cat %>%
  arrange(desc(Pourcentage)) %>%
  kable(
    caption = "Répartition de l’importance des 16 variables explicatives par catégorie",
    digits = 1,
    booktabs = TRUE,
    align = "lrrr"
  ) %>%
...
## Analyse des Partial Dependance Plots (PDP)
...
### Nombre de nouveaux usagers (nb_nouveaux_usagers)

Le PDP montre une relation fortement croissante entre l’arrivée de nouveaux usagers et la fréquentation quotidienne. L’effet est quasi linéaire jusqu’à environ 50 nouveaux usagers, puis tend à se stabiliser. Ce palier suggère un effet de saturation progressive, indiquant que l’impact marginal de chaque nouvel usager diminue au-delà d’un certain seuil. Ce résultat confirme que l’acquisition de nouveaux usagers est un levier structurant, mais qu’elle doit être accompagnée de mécanismes de rétention pour maintenir son efficacité.

### Nombre d’espaces actifs (nb_espaces_actifs)

L’impact du nombre d’espaces actifs est marqué et non linéaire. Une rupture nette apparaît autour de 8 à 10 espaces actifs, au-delà de laquelle la fréquentation augmente significativement. Ce seuil suggère un effet de masse critique : tant qu’un nombre minimal d’espaces n’est pas activé simultanément, l’effet sur la fréquentation reste limité. L’activation coordonnée des espaces apparaît ainsi comme un levier clé de dynamisation du parc.

### Fréquentation récente (nb_visiteurs_ma7)

Le PDP associé à la moyenne mobile sur 7 jours met en évidence un fort effet d’inertie. La fréquentation augmente rapidement lorsque la fréquentation récente dépasse un certain niveau, avant d’atteindre un plateau. Ce comportement confirme l’existence d’un mécanisme auto-renforçant : un parc déjà fréquenté a plus de chances de continuer à l’être, traduisant un effet d’entraînement social et organisationnel.

### Jour de la semaine (jour_semaine_num)

L’effet du jour de la semaine est plus modéré mais clairement structuré. La fréquentation augmente progressivement en milieu de semaine, atteint un maximum en fin de semaine ouvrée, puis décroît. Ce profil confirme que les rythmes hebdomadaires influencent la fréquentation, mais de manière secondaire par rapport aux dynamiques internes du parc.


Pris ensemble, les PDP montrent que la fréquentation du Sèmè City Open Park est principalement gouvernée par des effets de seuil, d’inertie et de saturation, caractéristiques de dynamiques endogènes. Les leviers les plus puissants ne résident pas dans des facteurs exogènes fixes, mais dans la capacité du parc à activer suffisamment d’usagers, d’espaces et de dynamique collective simultanément.
...
# Discussion des hypothèses et conclusions de l'étude
...
L'Hypothèse H1 est donc validée.
...
```{r}
knitr::include_graphics("outputs/figures/courbe_survie_scop.png")
```

L'analyse de survie valide statistiquement ces observations (test log-rank, p < 0,0001). Les temps médians de survie confirment que la trajectoire d'engagement se joue très tôt : 30 jours pour les Explorateurs, 120 jours pour les Réguliers, et au-delà de 240 jours pour les Fidèles. Une fois le seuil de 30 jours franchi, la probabilité de rétention augmente significativement.

La matrice de transition des états d’engagement apporte un éclairage dynamique essentiel à l’hypothèse H2. Elle montre que les trajectoires d’usage du Sèmè City Open Park ne sont ni linéaires ni figées, mais marquées par des passages fréquents entre différents niveaux d’engagement.

```{r}
knitr::include_graphics("outputs/figures/64_matrice_de_transition.png")
```

Les usagers récemment inscrits, souvent classés comme occasionnels ou explorateurs, constituent un carrefour stratégique : ils peuvent évoluer vers des profils fidèles, mais aussi basculer rapidement vers le churn. Ces transitions confirment que la première expérience vécue dans le parc ne détermine pas seulement le retour immédiat, mais influence durablement la trajectoire d’engagement.
...
L'Hypothèse H2 est également fortement validée.
...
Les leviers identifiés sont majoritairement internes, maîtrisables et à priori, peu coûteux à mettre en place. Les simulations montrent qu’en combinant ces actions, le parc pourrait théoriquement générer jusqu’à 246 000 visites supplémentaires, soit +1 710 % par rapport à la situation actuelle.

Ce chiffre doit être interprété comme une borne haute, illustrant le potentiel latent du lieu, et non comme une projection mécanique des visites futures.
...
Actions à réaliser dès aujourd’hui :

- formaliser un parcours d’accueil lisible pour les primo-visiteurs ;
- mettre en place un message de suivi systématique à J+2 par exemple après la première visite ;
...
- création de parcours de découverte, avec des activités et expériences immersives reliant plusieurs espaces ;
...
- privilégier une programmation d'activités légégères sur les créneaux creux (après 18h en semaine) ;
...
Comme toute analyse fondée sur des données empiriques, cette étude présente un certain nombre de limites qu’il convient d’expliciter. Les identifier ne remet pas en cause la validité des résultats obtenus ; au contraire, cela permet d’en préciser la portée et d’ouvrir des perspectives d’amélioration pour les travaux futurs. Elles soulignent surtout qu'au SCOP, la donnée est un chantier vivant, dont l’amélioration progressive conditionnera la capacité du parc à mieux se comprendre, à mieux se piloter et à mieux se projeter.

## Une vision partielle de la réalité observable
...
À titre d’exemple, seuls 224 étudiants de Africa Design School (toutes années confondues) apparaissent dans la base, alors que plus de 300 étudiants sont inscrits uniquement pour l’année académique 2025, toutes filières confondues, et bien plus encore des alumnis de l'Ecole fréquentent le SCOP. De même, les enfants participant aux programmes du TinIMA ne sont pas intégrés au dispositif de suivi, alors même qu’ils occupent une place centrale dans la vie du parc.
...
- des événements externes (examens, vacances, contexte socio-économique, météo, catastrophes politiques ou naturelles) ;
...
L’ensemble de ces limites converge vers une cinquième recommandation structurante : la nécessité de mettre en place un outil unifié de gestion des données au Sèmè City Open Park.

Aujourd’hui, les informations relatives aux usagers, aux activités et aux usages sont dispersées entre plusieurs outils hétérogènes, rendant difficile la reconstitution complète de l’expérience vécue par chaque usager au sein du parc.

Un système unifié de collecte et gestion des données permettrait :

- de créé une mémoire collective de l'historique du parc ;
- de retracer le parcours complet de chaque usager (inscription, visites, activités, fidélisation) ;
